...
export default function GamePage() {
    ...
    const {
        ...
        loading,
        updateUrl,
    } = useFetchSolution(`https://api.rawg.io/api/games/${id}?key=9269195f491e44539d7a2d10ce87ab15`);

    useEffect(() => {
        updateUrl(`https://api.rawg.io/api/games/${id}?key=9269195f491e44539d7a2d10ce87ab15`);
    }, [id]);

    return (
        <>
            {loading && <h1 className="text-indigo-300 text-xl text-center mt-10">Loading...</h1>}
                ...
                <section className="max-w-6xl mx-auto px-4 py-16 text-white" style={{ marginTop: 50 }}>
                    ...
                    <div className="flex flex-col md:flex-row gap-10 mb-12">
                        <div className="flex-shrink-0 w-full md:w-1/2 h-[400px] rounded-2xl overflow-hidden shadow-lg relative">
                            <img src={data.background_image} alt={data.name} className="w-full h-full object-cover opacity-90" />
                        ...
                        <div className="flex flex-col justify-center gap-6 md:w-1/2">
                            ...
                            {data.developers?.length > 0 && (
                                <p><strong className="text-indigo-400">Developer:</strong> {data.developers.map(d => d.name).join(", ")}</p>
                            )}

                            {data.publishers?.length > 0 && (
                                <p><strong className="text-indigo-400">Publisher:</strong> {data.publishers.map(p => p.name).join(", ")}</p>
                            )}

                            {data.esrb_rating && (
                                <p><strong className="text-indigo-400">Age rating:</strong> {data.esrb_rating.name}</p>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col md:flex-row gap-10 mb-12">
                        <div className="md:w-2/3">
                            ...
                            <p className="text-gray-300 leading-relaxed text-justify text-sm">{data.description_raw}</p>
                        </div>
